    ...
    <div class="container mx-auto px-4 py-10 md:py-12">
        <div class="p-8 bg-white my-10 border border-gray-300 rounded rounded-t-lg ">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Entire {{ appName() }} Transaction History
            </h2>
            ...
            <div name="description">
                ...
                <div class="grid grid-cols-12 w-full mb-5 pb-3 rounded">
                    ...
                    <div class="col-span-12 grid grid-cols-12">
                        @forelse($ts as $t)
                            <div
                                ...
                                @if($t->amount > 0)
                                    {{number_format($t->amount)}} @else {{ "(".number_format(abs($t->amount)).")" }}
                                @endif
                            </div>
                            @if ($t->user && $t->user->company)
                                <div class="col-start-2 col-span-2 mx-2 my-0 text-left text-xs  py-0 truncate">
                                    {{ data_get($t->user->company, 'name') }}
                                </div>
                            @endif

                            <div class="col-start-4 col-span-4 mx-2 text-xs text-gray-600 text-left  my-0 py-0">
                                <a>
                                    @if($t->type == 1)
                                        {{ data_get($t->point->reciever, 'name')}} received @if($t->point->is_super) super @endif
                                        {{ getReplacedWordOfKudos() }} from {{ giverName($t->point) }}
                                    @elseif($t->type == 2)
                                        {{ data_get($t->user, 'name') }} {{ $t->note }}
                                    @else
                                        {{$t->note}}
                                    @endif
                                </a>
