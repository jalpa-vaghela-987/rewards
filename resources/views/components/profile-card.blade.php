<div
    class="w-full h-full rounded rounded-t-lg overflow-hidden bg-white border border-gray-300 px-8 py-2 self-start  flex flex-col justify-between"
    ...
    @if(auth()->check() && auth()->user()->role === 1)
        <div class="text-left text-sm mt-3 border-gray-200 border-t -my-2 p-2 text-gray-600">
            ...
            <p class="text-xs mt-1 text-center">{{ number_format($user->points_to_give) }} {{ getReplacedWordOfKudos() }} available
                to give away</p>
        </div>
    ...
    @endif
        ...
        @else
            <div class="flex justify-center">
                <img src="{{ appFavicon() }}" class="rounded-full hover:bg-gray-100 h-16">
            </div>

            <div class="text-center px-3 py-1 mt-1">
                <a class="text-black text-xls bold font-sans py-1 px-2 cursor-default" href="javascript:void(0)">
                    {{ config('app.name') }}
                </a>
            </div>
    @endif
